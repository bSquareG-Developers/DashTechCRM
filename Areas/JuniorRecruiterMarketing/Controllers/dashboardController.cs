...
namespace DashTechCRM.Areas.JuniorRecruiterMarketing.Controllers
{
    public class dashboardController : Controller
    {
        ...
        {
            UserObject user = Session["userInfo"] as UserObject;
            string query = string.Format(@"select CA.AssignedId,CM.CandidateId,UADRecruiter.FullName,CM.CandidateName,CM.CandidateStatus,CM.VisaStatus,CM.Date as EnrolledDate,CM.MarketingStartDate,CMD.MarketingId,CMD.MarketingEmailId,CMD.MarketingContactNumber from CandidateAssign CA Inner join UserAccountDetails UADRecruiter on CA.JrBatchRecruiter = UADRecruiter.UserId INNER JOIN CandidateMarketingDetails CMD on CMD.MarketingId = CA.refMarketingId INNER JOIN CandidateMaster CM on CM.CandidateId = CMD.RefCandidateId where CA.JrBatchRecruiter={0} and CM.MarketingStartDate = '{1}' order by CA.AssignedId desc", user.UserId, DateTime.Now.Date.ToString("yyyy-MM-dd"));
            return CommonHelperClass._serializeDatatable(dl.GetDataTable(query));
        }
        #endregion
        ...
        public string SaveCandidateAssignToTraineeRecruiter(string parameter)
        {
            try
            {
                List<SqlParameter> p = new List<SqlParameter>();
                ...
                dynamic prm = JObject.Parse(parameter);
                p.Add(new SqlParameter("@TraineeBatchRecruiter", Convert.ToString(prm.traineeBatchRecruiter)));
                ...
                p.Add(new SqlParameter("@AssignedId", Convert.ToString(prm.AssignedId)));
                object result = dl.Execute_Scaler("CandidateAssign_TraineeBatchRecruiter", p.ToArray());
                return result.ToString();
            }
            catch (Exception e)
            {
